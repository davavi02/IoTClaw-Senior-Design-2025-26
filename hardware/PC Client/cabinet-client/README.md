...
For the other files:
- `testing_socket.py` - (OUTDATED) This creates a websocket server to emulate the backend websocket for testing purposes. It sends a uint8 data bit to the PC Client, like the backend.
...
- `obsmanager.py` - This has the code to talk to OBS websocket and cycle through different cameras.
...
Run the exe generated by pyinstaller or `python ./main.py`

## Development
1. Have Python 3.12.1 installed as it is what the PC has and install the following packages

    `pip install websockets requests pyserial python-opencv obs-websocket-py pyinstaller`

    (Recommended) If you have the uv python manager:
    
    `uv sync` on the cabinet-client directory

2. Run `pyinstaller --onefile main.py` to build an exe for the program
